...
class M9BackupManager:
    """M9 开发工坊备份管理器.

    封装统一备份管理器，提供 M9 专用的备份恢复接口。
    ...
    def __init__(self, data_dir: Optional[str] = None):
        ...
        if data_dir:
            ...
            # 从 data_dir 计算数据库路径
            self._db_path = str(self._data_dir / "yunxi_m9.db")
        else:
        ...
        # 模块ID
        self._module_id = "m9_dev_workshop"
...
_backup_manager: Optional[M9BackupManager] = None


def get_backup_manager() -> M9BackupManager:
    """获取 M9 备份管理器全局单例.
    ...
    if _backup_manager is None:
        _backup_manager = M9BackupManager()
    return _backup_manager


def init_backup_manager(data_dir: Optional[str] = None) -> M9BackupManager:
    """初始化备份管理器（可用于自定义配置）.
    ...
    """
    global _backup_manager
    _backup_manager = M9BackupManager(data_dir=data_dir)
    return _backup_manager
...
def main():
    ...
    parser = argparse.ArgumentParser(description="M9 开发工坊 - 备份管理")
    ...
    if args.command == "backup":
        print("[M9-Backup] 开始备份...")
        report = bm.backup_now()
        print(f"[M9-Backup] 备份完成:")
        ...
        backups = bm.list_backups()
        print(f"[M9-Backup] 备份列表（共 {len(backups)} 个）:")
