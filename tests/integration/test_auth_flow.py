...
注意：所有测试均需要 M8/M11 后端服务运行，标记为 @pytest.mark.integration。
...
class TestAuthFlowIntegration:
    ...
    def test_m8_full_auth_flow(self, m8_client):
        ...
        assert login_data.get("code") == 0, \
            f"登录业务失败: {login_data.get('message')}"

        token_data = login_data.get("data", {})
        access_token = token_data.get("access_token") or token_data.get("token")
        assert access_token, "未获取到 access_token"
        ...
        assert logout_response.status_code in [200, 404], \
            f"登出接口返回意外状态码: {logout_response.status_code}"
    ...
    def test_m8_token_refresh(self, m8_client):
        ...
        login_data = login_response.json()
        assert login_data.get("code") == 0, "登录业务失败"
        ...
        assert refresh_response.status_code in [200, 401, 404], \
            f"Token 刷新接口返回意外状态码: {refresh_response.status_code}"
    ...
    def test_m8_expired_token_rejected(self, m8_client):
        ...
        # 过期 token 应该返回 401 或 403
        assert response.status_code in [401, 403], \
            f"过期 Token 未被拒绝，返回状态码: {response.status_code}"
    ...
    def test_m8_user_info_after_login(self, m8_client):
        ...
        token_data = login_data.get("data", {})
        access_token = token_data.get("access_token") or token_data.get("token")
        assert access_token, "未获取到 token"
        ...
        assert me_response.status_code == 200, \
            f"获取用户信息失败，状态码: {me_response.status_code}"
    ...
    def test_m11_apikey_auth_flow(self, m11_client):
        ...
        assert no_key_response.status_code in [401, 403], \
            f"无 API Key 访问未被拒绝，状态码: {no_key_response.status_code}"
        ...
        assert with_key_response.status_code in [200, 401, 403, 404], \
            f"API Key 访问返回意外状态码: {with_key_response.status_code}"
    ...
    def test_m11_multiple_auth_methods(self, m11_client):
        ...
        # 验证所有方法都返回了状态码
        assert len(results) == 3, f"部分认证方式未执行成功: {results}"
