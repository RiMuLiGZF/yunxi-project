...
class EqProgressUpdateRequest(BaseModel):
    progress: int
...
async def search_contacts(
    ...
    if repo:
        try:
            contacts = repo.search_contacts(q)
            result = [c.to_dict() for c in contacts]
            return {"code": 0, "message": "ok", "data": result}
        except Exception as e:
            print(f"[Social] contacts search 查询失败，使用 fallback: {e}")

    # 内存 fallback
    result = [c for c in _contacts if q.lower() in c["name"].lower()]
    return {"code": 0, "message": "ok", "data": result}
...
async def get_interaction_detail(
    ...
    if repo:
        try:
            interaction = repo.get_interaction(iid)
            if not interaction:
                return {"code": 404, "message": "交往记录不存在", "data": None}
            return {"code": 0, "message": "ok", "data": interaction.to_dict()}
        except Exception as e:
            print(f"[Social] interaction detail 查询失败，使用 fallback: {e}")

    # 内存 fallback
    interaction = next((i for i in _interactions if i["id"] == iid), None)
    if not interaction:
        return {"code": 404, "message": "交往记录不存在", "data": None}
    return {"code": 0, "message": "ok", "data": interaction}


@router.put("/interactions/{iid}")
async def update_interaction(
    ...
    if repo:
        try:
            update_data = req.dict(exclude_unset=True)
            interaction = repo.update_interaction(iid, **update_data)
            if not interaction:
                return {"code": 404, "message": "交往记录不存在", "data": None}
            return {"code": 0, "message": "更新成功", "data": interaction.to_dict()}
        except Exception as e:
            print(f"[Social] update_interaction 失败，使用 fallback: {e}")
    ...
    return {"code": 0, "message": "更新成功", "data": interaction}


@router.delete("/interactions/{iid}")
async def delete_interaction(
    ...
    if repo:
        try:
            success = repo.delete_interaction(iid)
            if not success:
                return {"code": 404, "message": "交往记录不存在", "data": None}
            return {"code": 0, "message": "删除成功", "data": None}
        except Exception as e:
            print(f"[Social] delete_interaction 失败，使用 fallback: {e}")

    # 内存 fallback
    global _interactions
    original_len = len(_interactions)
    _interactions = [i for i in _interactions if i["id"] != iid]
    if len(_interactions) == original_len:
        return {"code": 404, "message": "交往记录不存在", "data": None}
    return {"code": 0, "message": "删除成功", "data": None}
...
async def get_reminders(
    ...
    if repo:
        try:
            reminders = repo.list_reminders(status=status, reminder_type=reminder_type, priority=priority)
    ...
    if reminder_type:
        result = [r for r in result if r.get("type") == reminder_type]
    if priority:
...
@router.get("/eq-courses/{lid}")
async def get_eq_course_detail(
    lid: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
    ...
    if repo:
        try:
            lesson = repo.get_eq_lesson(lid)
            if not lesson:
                return {"code": 404, "message": "课程不存在", "data": None}
            return {"code": 0, "message": "ok", "data": lesson.to_dict()}
        except Exception as e:
            print(f"[Social] eq-course detail 查询失败，使用 fallback: {e}")

    # 内存 fallback
    course = next((c for c in _eq_courses if c["id"] == lid), None)
    if not course:
        return {"code": 404, "message": "课程不存在", "data": None}
    return {"code": 0, "message": "ok", "data": course}


@router.put("/eq-courses/{lid}/progress")
async def update_eq_course_progress(
    lid: int,
    req: EqProgressUpdateRequest,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
    """更新情商课程进度"""
    ...
    if repo:
        try:
            lesson = repo.update_eq_lesson_progress(lid, req.progress, req.completed_lessons)
            if not lesson:
                return {"code": 404, "message": "课程不存在", "data": None}
            return {"code": 0, "message": "进度更新成功", "data": lesson.to_dict()}
        except Exception as e:
            print(f"[Social] update_eq_lesson_progress 失败，使用 fallback: {e}")
    ...
    if req.completed_lessons is not None:
        course["completed_lessons"] = req.completed_lessons

    return {"code": 0, "message": "进度更新成功", "data": course}
