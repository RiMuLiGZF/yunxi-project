"""
M4 场景引擎 - M8 Token 安全修复测试

验证 _verify_m8_token 函数的安全行为：
- 生产环境 + 空 token → 拒绝
- 开发环境 + 空 token → 放行
- 有效 token → 通过
- 无效 token → 拒绝
- 空请求 token → 拒绝（配置了 token 的情况下）
...
# ============================================================================
# 从 main.py 源码中提取核心验证逻辑进行测试
# 避免导入整个 main 模块带来的复杂依赖
# ============================================================================
...
class TestM4VerifyM8Token:
    """M4 M8 Token 验证安全测试"""

    def test_production_env_empty_token_rejected(self, monkeypatch):
        """生产环境下 token 未配置时应拒绝访问"""
        monkeypatch.delenv("M4_ADMIN_TOKEN", raising=False)
        monkeypatch.setenv("YUNXI_ENV", "production")
        assert _verify_m8_token("") is False
        assert _verify_m8_token("any-token") is False

    def test_prod_env_empty_token_rejected_case_insensitive(self, monkeypatch):
        """生产环境大小写不敏感：PROD 也应拒绝"""
        monkeypatch.delenv("M4_ADMIN_TOKEN", raising=False)
    ...
    def test_valid_token_passes(self, monkeypatch):
        """配置了有效 token 时应通过验证"""
        monkeypatch.setenv("M4_ADMIN_TOKEN", "my-secret-token-123")
        monkeypatch.setenv("YUNXI_ENV", "production")
    ...
    def test_main_source_code_matches(self):
        """验证 main.py 中的实际实现与测试逻辑一致

        检查 main.py 源码中包含正确的安全模式：
        - 生产环境判断
        - YUNXI_ENV 环境变量
        - hmac.compare_digest
        """
