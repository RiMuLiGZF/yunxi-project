"""
云汐内核 V10.0 — 全量子Agent集成测试套件

覆盖8个子Agent + 分身池的核心功能，含：
- 共享数据模型测试
- TaskDAG拓扑/关键路径/完成率测试
- 各子Agent handle_task功能测试
- 分身池生命周期测试
- 并发安全测试
"""
...
from shared_models import (
    TaskDAG, DAGNode, DAGEdge, AgentRole, SecurityClassification,
    AgentLifeState, SchedulingDecision, ArbitrationLevel, CloneType,
    SubAgentIdentity, LoadScore, TeamComposition, ArbitrationRequest,
    ArbitrationResult, CloneIdentity,
)
...
class TestLifecycleAgent:
    ...
    def test_instance_pool_ref_count(self):
        ...
        # create不自动设置ref_count，需要手动add_ref
...
class TestDiscoveryAgent:
    ...
    def test_scheduling_policy_cloud_no_network(self):
        policy = SchedulingPolicy()
        # LOCAL_FIRST策略：电量充足+无网络+低复杂度→LOCAL_FIRST
        decision = policy.decide(battery_pct=90.0, network_available=False, task_complexity=0.3)
        assert decision == SchedulingDecision.LOCAL_FIRST
...
from arbiter.wait_for_graph import WaitForGraph, ArbitrationEngine
...
class TestArbiterAgent:
    ...
    @pytest.mark.asyncio
    async def test_arbitrate_resource_deadlock(self, agent):
        # resource_deadlock 可能在level1自动解决
        result = agent.arbitrate(
            conflict_type="resource_deadlock",
            involved_agents=["agent.1", "agent.2"],
            task_ids=["task.1"],
            context={"priority": {"agent.1": 1, "agent.2": 5}},
        )
        assert result.decision != ""

    def test_arbitration_engine_timeout(self):
        engine = ArbitrationEngine()
        # timeout + agent_info 使level1可以解决
...
class TestClonePool:
    ...
    def test_create_clone(self, factory):
        ...
        assert "full_spec" not in clone.minimized_context  # 最小信息原则
    ...
    async def test_pool_limits(self, pool):
        ...
        clones = []
        for i in range(5):
            c = pool.acquire("agent.1", CloneType.SCOUT, f"task.{i}", {"desc": f"任务{i}"})
            if c:
                clones.append(c)
        stats = pool.stats()
        assert stats["total_clones"] <= pool._max_clones_per_parent

    @pytest.mark.asyncio
    async def test_cleanup_expired(self, pool):
        await pool.acquire("agent.1", CloneType.SCOUT, "task.1", {"desc": "test"})
