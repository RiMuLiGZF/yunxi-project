...
形象工坊已迁移到 M4 场景引擎（迁移脚本：migrate_emotion_appearance_m8_to_m4.py）

原本地实现已迁移至 M4 场景引擎，当前文件为代理路由占位。
业务请求通过 m4_proxy_middleware 中间件转发到 M4。

代理路径映射：
  M8 /api/appearance/*  →  M4 /api/v1/appearance/*
...
@router.get("/health", summary=f"形象工坊服务状态（M4代理）")
async def appearance_health(
    ...
    try:
        ...
        return ApiResponse.success(data={
            ...
            "target_prefix": "/api/v1/appearance",
    ...
    try:
        ...
        return ApiResponse.success(data={
            **status,
            "service": "appearance",
            "service_name": "形象工坊",
            "m8_prefix": "/api/appearance",
            "m4_prefix": "/api/v1/appearance",
