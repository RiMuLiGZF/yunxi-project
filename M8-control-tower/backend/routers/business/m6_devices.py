...
穿戴设备管理已迁移到 M6 硬件外设模块（迁移脚本：migrate_wearable_m8_to_m6.py）

原本地实现已迁移至 M6 模块。
所有请求通过路由层直接代理转发到 M6。

代理路径映射：
  M8 /api/v1/m6/*  →  M6 /api/v1/*
...
# M6 服务配置
MODULE_BASE_URL = os.environ.get("M6_BASE_URL", "http://localhost:8006")
MODULE_ADMIN_TOKEN = os.environ.get("M6_ADMIN_TOKEN", "")
MODULE_TIMEOUT = float(os.environ.get("M6_TIMEOUT", "30"))
...
@router.get("/health", summary=f"M6穿戴设备管理服务状态（M6代理）")
...
async def generic_proxy(
    ...
    # 构建目标路径
    target_path = f"/api/v1/{path}" if path else "/api/v1"
    ...
    try:
        async with _get_client() as client:
            ...
            response_headers["X-M6-Proxied"] = "true"
        ...
        return JSONResponse(
            status_code=503,
            content={"code": 503, "message": "M6 服务不可用", "data": None},
        )
    except httpx.TimeoutException:
        return JSONResponse(
            status_code=504,
            content={"code": 504, "message": "M6 请求超时", "data": None},
        )
    except Exception as e:
        return JSONResponse(
            status_code=502,
            content={"code": 502, "message": f"M6 代理失败: {str(e)}", "data": None},
        )
