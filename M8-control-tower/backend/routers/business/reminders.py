...
主动提醒功能迁移到 M4 场景引擎（路由层代理）

原本地实现已迁移至 M4 模块。
所有请求通过路由层直接代理转发到 M4。

代理路径映射：
  M8 /api/reminders/*  →  M4 /api/v1/reminders/*
...
@router.get("/health", summary=f"主动提醒服务状态（M4代理）")
...
async def proxy_info(
    ...
    return ApiResponse.success(data={
        ...
        "target_prefix": "/api/v1/reminders",
        "m8_prefix": "/api/reminders",
        "timeout": MODULE_TIMEOUT,
        "migrated": True,
...
async def generic_proxy(
    ...
    """通用代理接口，将请求转发到 M4 /api/v1/reminders/<path>"""
    user_id = "default"
    if current_user:
    ...
    # 构建目标路径
    target_path = f"/api/v1/reminders/{path}" if path else "/api/v1/reminders"
