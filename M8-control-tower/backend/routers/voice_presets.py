...
音色管理已迁移到 M4 场景引擎（语音服务子模块）

原本地实现已迁移至 M4 场景引擎，当前文件为代理路由占位。
业务请求通过 m4_proxy_middleware 中间件转发到 M4。

代理路径映射：
  M8 /api/voice/presets/*  →  M4 /api/v1/voice/presets/*
...
@router.get("/health", summary=f"音色管理服务状态（M4代理）")
async def voice_presets_health(
    ...
    try:
        ...
        return ApiResponse.success(data={
            ...
            "target_prefix": "/api/v1/voice/presets",
    ...
    try:
        ...
        return ApiResponse.success(data={
            **status,
            "service": "voice_presets",
            "service_name": "音色管理",
            "m8_prefix": "/api/voice/presets",
            "m4_prefix": "/api/v1/voice/presets",
