...
封装情绪陪伴模块的数据库 CRUD 和查询。
数据库优先，失败时自动降级到内存模式（保持兼容性）。
...
class EmotionRepository:
    ...
    def _seed_default_contents(self):
        """初始化默认内容库（幂等操作）"""
        # 放松内容
        relax_count = self.db.query(RelaxContent).count()
        if relax_count == 0:
            for item in _DEFAULT_RELAX_CONTENTS:
                self.db.add(RelaxContent(**item))
            self.db.commit()
            print(f"[EmotionRepo] 初始化放松内容: {len(_DEFAULT_RELAX_CONTENTS)} 条")

        # 助眠内容
        sleep_count = self.db.query(SleepContent).count()
        if sleep_count == 0:
            for item in _DEFAULT_SLEEP_CONTENTS:
                self.db.add(SleepContent(**item))
            self.db.commit()
            print(f"[EmotionRepo] 初始化助眠内容: {len(_DEFAULT_SLEEP_CONTENTS)} 条")

        # 心理测评
        assess_count = self.db.query(PsychAssessment).count()
        if assess_count == 0:
            for item in _DEFAULT_ASSESSMENTS:
                questions = item.pop("questions")
                self.db.add(PsychAssessment(
        ...
        # 示例情绪记录（仅当完全没有记录时）
        emotion_count = self.db.query(EmotionRecord).filter(
            EmotionRecord.user_id == self.user_id
        ).count()
        if emotion_count == 0:
            self._seed_sample_emotion_records()

        # 示例心情日记（仅当完全没有时）
        diary_count = self.db.query(MoodDiary).filter(
            MoodDiary.user_id == self.user_id
        ).count()
        if diary_count == 0:
            self._seed_sample_mood_diary()

    def _seed_sample_emotion_records(self):
        """生成示例情绪记录（30天）"""
    ...
    def _seed_sample_mood_diary(self):
        """生成示例心情日记"""
        samples = [
    ...
    def get_emotion_records(self, days: int = 30) -> List[EmotionRecord]:
        """获取情绪记录（最近 N 天）"""
        ...
        return (
            ...
            .filter(
                EmotionRecord.user_id == self.user_id,
                EmotionRecord.date >= start_date,
            )
            .order_by(EmotionRecord.date.asc())
            .all()
        )
    ...
    def get_overview_stats(self) -> Dict[str, Any]:
        ...
        total = (
            self.db.query(EmotionRecord)
            .filter(EmotionRecord.user_id == self.user_id)
            .count()
        )
    ...
    def record_emotion(self, emotion: str, level: int, trigger: str = "", note: str = "") -> EmotionRecord:
        """记录情绪（今日已有则更新）"""
        today = datetime.now().strftime("%Y-%m-%d")
        existing = (
            self.db.query(EmotionRecord)
    ...
    def get_relax_contents(self, category: Optional[str] = None) -> List[RelaxContent]:
        """获取放松内容列表"""
        query = self.db.query(RelaxContent)
        if category:
            query = query.filter(RelaxContent.category == category)
        return query.order_by(RelaxContent.id.asc()).all()

    def get_relax_content(self, content_id: int) -> Optional[RelaxContent]:
        """获取放松内容详情"""
        return self.db.query(RelaxContent).filter(RelaxContent.id == content_id).first()

    # ---------- 助眠内容 ----------

    def get_sleep_contents(self, category: Optional[str] = None) -> List[SleepContent]:
        """获取助眠内容列表"""
        query = self.db.query(SleepContent)
        if category:
            query = query.filter(SleepContent.category == category)
        return query.order_by(SleepContent.id.asc()).all()

    # ---------- 心理测评 ----------

    def get_assessments(self) -> List[PsychAssessment]:
        """获取测评列表"""
        return self.db.query(PsychAssessment).order_by(PsychAssessment.id.asc()).all()

    def get_assessment(self, assessment_id: int) -> Optional[PsychAssessment]:
        """获取测评详情"""
        return self.db.query(PsychAssessment).filter(PsychAssessment.id == assessment_id).first()

    def get_assessment_results(self) -> List[AssessmentResult]:
        """获取测评历史"""
        return (
            self.db.query(AssessmentResult)
            .filter(AssessmentResult.user_id == self.user_id)
    ...
    def get_mood_entries(self, emotion: Optional[str] = None) -> List[MoodDiary]:
        """获取心情日记列表"""
        query = self.db.query(MoodDiary).filter(MoodDiary.user_id == self.user_id)
        if emotion:
            query = query.filter(MoodDiary.mood == emotion)
        return query.order_by(desc(MoodDiary.created_at)).all()

    def create_mood_entry(self, emotion: str, content: str, tags: List[str] = None) -> MoodDiary:
        """创建心情日记"""
        entry = MoodDiary(
            mood=emotion,
            content=content,
