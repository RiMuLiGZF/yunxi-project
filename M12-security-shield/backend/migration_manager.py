...
封装统一迁移引擎，提供 M12 模块专用的迁移管理接口。
使用 SQLAlchemyMigrationAdapter 适配 M12 的 SQLAlchemy 引擎。
...
def _get_project_root() -> str:
    ...
    # M12 模块根目录
    m12_dir = backend_dir.parent
    # 项目根目录
    project_root = m12_dir.parent
...
class M12MigrationManager:
    """M12 安全盾迁移管理器.

    封装统一迁移引擎，提供 M12 专用的迁移管理接口。
    ...
    def __init__(self, engine=None, db_path: Optional[str] = None):
        ...
        """
        # 延迟导入避免循环引用
        from .database import engine as sa_engine, settings

        self._sa_engine = engine or sa_engine
        self._db_path = db_path or str(settings.db_path)
        self._db_name = "m12_security_shield"
...
_migration_manager: Optional[M12MigrationManager] = None


def get_migration_manager() -> M12MigrationManager:
    """获取 M12 迁移管理器全局单例.

    Returns:
        M12MigrationManager 实例
    """
    global _migration_manager
    if _migration_manager is None:
        _migration_manager = M12MigrationManager()
    return _migration_manager


def init_migration_manager(engine=None, db_path: Optional[str] = None) -> M12MigrationManager:
    """初始化迁移管理器（可用于自定义配置）.
    ...
    """
    global _migration_manager
    _migration_manager = M12MigrationManager(engine=engine, db_path=db_path)
    return _migration_manager
...
def main():
    ...
    parser = argparse.ArgumentParser(description="M12 安全盾 - 数据库迁移管理")
    ...
    if args.command == "migrate":
        print(f"[M12-Migration] 开始迁移...")
        print(f"[M12-Migration] 当前版本: {mgr.get_current_version()}")
        print(f"[M12-Migration] 最新版本: {mgr.get_latest_version()}")

        if args.dry_run:
            print("[M12-Migration] DRY-RUN 模式（模拟执行）")
        ...
        history = mgr.get_migration_history()
        print(f"[M12-Migration] 迁移历史（共 {len(history)} 条）:")
        for m in history:
