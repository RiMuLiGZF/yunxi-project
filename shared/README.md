...
| **鉴权工具** | `auth.py` | API Key 管理、路径白名单、令牌桶限流、FastAPI 适配 |
...
**功能**：
- `hash_api_key(key)`：计算 API Key 的 SHA256 哈希值，用于安全存储
- `verify_api_key(key, valid_keys)`：验证 API Key，支持明文密钥列表和（哈希值, 元数据）元组两种格式
- `is_public_path(path, public_paths)`：路径白名单匹配，支持通配符 `*`
- `DEFAULT_PUBLIC_PATHS`：默认公开路径常量（含 /health、/docs、/m8/* 等）
- `SimpleRateLimiter`：线程安全的内存令牌桶限流器，支持按 key 分别限流
- `create_api_key_dependency()`：创建 FastAPI `Depends` 适配函数，集成 X-API-Key / Bearer 鉴权、公开路径放行、速率限制
- `generate_api_key(prefix, length)`：生成加密安全的随机 API Key，支持自定义前缀
- `mask_api_key(key, show_first, show_last)`：密钥脱敏，用于日志安全输出
...
# 验证密钥（支持明文和哈希两种格式）
valid_keys = [
    (key_hash, {"name": "admin", "permissions": ["*"]}),
    "other-secret-key",
]
meta = verify_api_key(key, valid_keys)

# 路径白名单
if is_public_path("/m8/health", ["/m8/*"]):
    pass  # 无需鉴权
