...
- 日志级别动态配置
- 敏感字段自动脱敏
- contextvars 链路追踪（trace_id / user_id / device_id）
- 日志文件轮转（可选）
"""
...
def set_context(**kwargs: str) -> None:
    ...
    Args:
        **kwargs: 上下文键值对，支持 trace_id / user_id / device_id / request_path.
    """
    ...
    if "device_id" in kwargs:
        device_id_var.set(kwargs["device_id"])
...
def _inject_contextvars(
    ...
    将当前上下文中的 trace_id / user_id / device_id / request_path
    自动注入到每条日志中。
