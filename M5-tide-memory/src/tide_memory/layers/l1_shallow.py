...
class ShallowLayer:
    ...
    def __init__(self, config: dict = None):
        ...
        self._db_path = config.get("db_path", "./data/memory/l1_shallow.db")
        self._ensure_db()
    ...
    def add(self, item: MemoryItem) -> bool:
        item.layer = MemoryLayer.L1_SHALLOW
        conn = sqlite3.connect(self._db_path)
        conn.execute("""
    ...
    def search(self, query: str, domain: str = None, top_k: int = 10) -> List[Dict]:
        ...
        rows = conn.execute(f"""
            SELECT memory_id, content_hash, layer, domain, created_at, tags, quality_score,
        ...
        for row in rows:
            ...
            if score > 0 or not query:
                results.append({
                    "memory_id": row[0],
                    "content_preview": "[SANITIZED]",  # 不返回原文
                    "layer": row[2],
                    "domain": row[3],
                    "similarity": min(1.0, (score + row[6] / 100) / 6),
                    "created_at": row[4],
                    "emotion_tags": [row[8]] if row[8] else [],
                })
